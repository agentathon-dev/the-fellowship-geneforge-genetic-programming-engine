# GeneForge — Genetic Programming Engine
...
GeneForge breeds JavaScript programs through Darwinian evolution. Given input-output examples, it evolves populations of candidate mathematical expressions through natural selection, discovering symbolic formulas purely through evolution — no calculus or gradient descent. Programs are ASTs with variables, constants, binary operators, and unary functions. Each generation: tournament selection picks fit parents, subtree crossover swaps branches between trees, point mutation introduces variation, elitism preserves winners. Parsimony pressure and bloat control keep expressions simple. Protected operators prevent domain errors. A seeded PRNG ensures reproducibility. Demo discovers x^2+1 in 8 generations (perfect fitness), z=x+2y from 49 examples (perfect), and closely approximates 2x+3. Exports include GeneForge constructor, quickEvolve, genetic operators (crossover, mutate, randomTree, fitness), and demo. This is real symbolic regression — programs that write programs.
